...
// Control Panel:
		...
		ImGui::Text("%.0f spp", (float)renderer.GetSPP());

		ImGui::Separator();
		
		const float sp = renderer.RR_survival_probability;
		ImGui::Text("Russian Roulette survival probability: %.1f", sp);
		...
		if (ImGui::Button("Russian Roulette survival probability = 90%"))
		{
			renderer.Reaccumulate();
			renderer.RR_survival_probability = 0.9f;
		}
		if (ImGui::Button("Russian Roulette survival probability = 80%"))
		{
			renderer.Reaccumulate();
			renderer.RR_survival_probability = 0.8f;
		}
		if (ImGui::Button("Russian Roulette survival probability = 50%"))
		{
			renderer.Reaccumulate();
			renderer.RR_survival_probability = 0.5f;
		}
